...
def text_to_image(text: str, id: int):
    ...
    payload = {
        ...
        "text": f"{text}", # Это наш промт, то есть текст, по которому мы получим катинку. Его мы передаем в саму функцию
    ...
    # Мы получили на ша ответ в json формате, тоесть в формате словаря
    ...
    # Насильно создаст папку iamges в директории с ботом, нужно для docker
...
async def command_image_handler(message: Message):
    ...
    try:
        ...
        # ы ее сохраняем как перемнную "e"
        ...
        print(e) # Выведем ошибку в консоль для понимания, что случилось,
        # чтобы исправить
    await edited.delete()